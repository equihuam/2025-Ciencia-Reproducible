...
function SetLookup(strParameter, farrXvals, farrYvals)
{
	...
	let rval = Module.ccall('SetLookup', 'number', ['number','number','number','number','number'], [-1, ptr, xarr.length, bufX, bufY]);
...
{
	_DumpResultsToDisk();
	DownloadFileFromWASMVirtualFileSystem("export.csv");
}

function DownloadFileFromWASMVirtualFileSystem(strFileName)
{

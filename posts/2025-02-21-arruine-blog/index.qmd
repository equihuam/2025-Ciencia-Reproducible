...
Como ya nos pasó, puede ser que eches a perder tu archivo _index.qmd_ en la raíz de tu proyecto. Ese documento se encarga de construir la estructura general de tu _blog_, defina la llamada **landing page** de tu proyecto, la cara del primer contacto de tu blog. Ojalá hayas echo un **commit** inicial justo acabando de crear tu proyecto. Si fuiste así de sabia o sabio, todo lo que tienes que hacer es buscar el archivo en cuestión y recuperarlo. **Github desktop** será tu amigo para eso o en la ventana de comandos de _RStudio_, la instrucció `git restore`, ve la sección siguiente.

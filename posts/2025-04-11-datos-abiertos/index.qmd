...
Los datos de las estaciones existentes los puedes consultar con la misma **API** o en la página del [Sinaica](https://sinaica.inecc.gob.mx/), Hay un enlace a [Infraestructura de los SMCA][sinaica-estaciones], que contiene los descriptores de cada una de las estaciones existentes en el país. Para hacerlo con la **API** buscamos en la tabla anterior _sinaica-estaciones_. Hacemos la petición así:

[sinaica-estaciones]:https://sinaica.inecc.gob.mx/archivoEstaciones.php
...
Las variables que puedo usar para construir esta **consulta** las encuentro precisamente en la columna _variables_ del catálogo. Para el caso de _Sinaica_ esta columna reporta las siguientes variables disponibles.
...
Como habíamos visto en [tipos de estudios](../2025-03-21-tipo-de estudio/index.qmd)
Puede interesarte también ete artíulo sobre los datos abiertos que ofrece [Inegi y la bilioteca para **R**][inegi-r] que han desarrollado. Este otro es un ejemplo detallado de [acceso y procesamiento de microdatos][microdatos-lasalle]. Algo semejante ofrece este artículo de @castro_usando_2021 y está también el libro completo en línea d @escoto_castillo_encuesta2021, sobre como usar microdato para analizar el mercado de tabajo.

[microdatos-lasalle]: https://revistasinvestigacion.lasalle.mx/index.php/relais/article/download/4122/3778/27670
[inegi-r]:https://rde.inegi.org.mx/index.php/2020/12/02/importinegi-un-paquete-de-r-para-descargar-y-gestionar-bases-de-datos-del-inegi/


## Precios según el **SNIIM**

Encontré que la forma de reportar los datos en la página del **SNII** es un poco complicada en términos informáticos. Lo que hacen es generar en forma dinámica una tabla con la i formación solicitada, pero en lugar de poner los datos en una tabla directamente, producen un **iframe**, es decir ofreven sólo una especie de _visor_ a datos que están en otro lado, aunque al desplegar el reporte producido a petición del usuario, parecería que está todo en un sólo lugar.

Para obtener los datos propiamente y poderlos leer a una tabla en **R**, es mejor hacer una cosulta directamente a la base de datos. Para eso necesitamos algunos datos específicos que obtendremos de los siguientes catálogos que con un poco de maña se pueden obtener del propio sitio del **SNIIM**, yo ya lo hice por tí y aquí están. A continuación podrás consultar estos catálogos para que contruyas peticiones a tu gusto.

\ 

```{r}
pacman::p_load(rvest, DT, dplyr, stringr, ggplot2)
...
Hagamos un ejercicio con los precios del _limón sin semilla_ en Xalapa, entre las fechas 01/feb/2025 y 01/abr/2025. Para hacer esto necesitamos los siguientes datos.

>**url base:** http://www.economia-sniim.gob.mx/nuevo/Consultas/MercadosNacionales/PreciosDeMercado/Agricolas/  \
>**escritp de consulta:**ResultadosConsultaFechaFrutasYHortalizas.aspx \
>**generador de la tabla de datos:** ResultadosConsultaFechaFrutasYHortalizas.aspx\   
>**fechaInicio:** 01/01/2025\ 
>**fechaFinal:** 01/04/2025\ 
>**ProductoId** 426\ 
>**OrigenId** 30\ 
>**Origen** Veracruz\ 
>**DestinoId** 301\ 
>**Destino** Veracruz: Central de Abasto de Jalapa\ 
>**PreciosPorId** 2 (este código se refiere a kg unitario)\ 
>**RegistrosPorPagina** 500 \ 

Ahora, semejante a los ejemplos anteriores, tomamos la **URL base** y, en este caso, la juntamos con la llamada al escript que hará la consulta a la base de datos y le ponemos una iterrogación al final. luego hacemos la consulta que nos interesa, agregando los datos separados por el símbolo **&**, así:
...
En general o conviene usar espacioes entre palabras en las **URL**, así que se suelen substituir por el código **%20**. Si tu consulta llegara a fallar, esta es una cosa que podrías intentar, pero antes checa que todo se vea según tu plan. Por cierto, notarás que cada elemento de la consulta tiene un nombre de parámetro y un valor. Normalmente esto sugiere que el orden en el que pongas las cosas no importa, salvo la parte ab
ntes de la interrogación.

Listo, hagamos la consulta para averiguar el patrón de precios del _limón sin semilla_ en lo que va de 2025 en Xalapa, Ver.
...
tabla_precios |> 
  ggplot(aes(x = feha_d, y = precio_num)) +
    ...
    labs(title = "Precio de limón sin semilla", 
         subtitle = "Xalapa, Ver. entre 01/ene/2025 y 01/abr/2025") +
    xlab(label = "Fecha") +

...
Para seguir con el ejercicio necesitamos crear una base de datos nueva. Eneste caso usaremos datos de ejemplo. Veamos la base de datos de los pasajeros del Titanic en su trágico viaje inaugural. Lo primero es preparar a _PostgreSQL_ para alojarla.
...
**home**, (asocido al símbolo **) quizás en _db-ejemplos_. Hay que usra `wget` con la opción **-P** seguida de la ruta deeada para almaenar el xontenido de la URL. Si los directorios involucrados existen, los usa, si no, los crea.
...
\ 
¿Había algún mexicano en el Titanic?
\ 

```{r}
mx_df <- dbGetQuery(connec, "SELECT name, hometown FROM passenger
                               WHERE hometown ILIKE '%mex%';")

print("Aquí está la resuesta\n")
peint(mx_df)

```

Hay que cerral la conexión

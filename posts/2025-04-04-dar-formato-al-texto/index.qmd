...
Voy a empezar por mi propia reacción "no me gustó la configuración *default* que usa *Quarto*", en cuanto a justificar a la izquierda las leyendas de las figuras. Para cambiar eso use el código del @lst-css. 
...
Para cambiar el tema modifica el archivo **\_quarto.yml** en la línea de theme. Por cierto, en la figura aparece la opción _lightbox_, que activa la función que permite interactuar con las figuras como entidades separadas, para que el usuario pueda explorarlas con más detalle.

![](img/tema.png){width=300}
  ...
  diagram =  "
      digraph DAG {
        ...
        node [shape = box,
              ...
              style = filled,
              fillcolor = cyan]
        tuberculosis_o_cancer; 

      
        # patrón de 'influencia'
        Visita_Asia->Tuberculosis
        Tuberculosis->tuberculosis_o_cancer
        tuberculosis_o_cancer->Rayos_X
        tuberculosis_o_cancer->Disnea
        Fumador->Cancer_pulmonar
        Fumador->Bronquitis
        Cancer_pulmonar->tuberculosis_o_cancer
        Bronquitis->Disnea
      }
      "
  )

...
Una ves que lo hayas instalado aparecerá un nuevo directorio en la raíz de tu proyecto con el nombre `_extensions` y también un archivo que te servira de *templete* con el nombre *template-full.qmd* y otro con los datos de las referencias: *references.bib*. Muévelos por favor a *posts*, a un subdirectorio apropiado para que mantengas el orden de tu trabajo. Para que experimentes un poco sobre la flexibilidad de lo que estamos haciendo, agrega lo siguiente en la sección de formato del *encabezado yaml* del *templete*, arribita de *academic-typst*:
...
La idea es que experimentes con la preparación de un reporte técnico con *Quarto*. Para averiguar un poco más sobre esto [te invito a visitar este documento](https://quarto.org/docs/manuscripts/authoring/rstudio.html). Incluso podrías interesarte en preparar un [libro completo](https://quarto.org/docs/books/) ¡Quizás es algo que podrías querer hacer para producir el documento de tu tesis! Bueno, por lo pronto lo que sigue es que construyas tu reporte, siguiendo ideas de formateo que te sugiere el *templete*. [La idea es algo así](articulo-pizza.qmd). Ejercita también la inclusión de [referencias bibliográficas](https://quarto.org/docs/authoring/citations.html). Resulta muy cómodá para esto la opción de insertar referencias que ofrece RStudio en su modo visual (@fig-citas) .

![](img/citas.png){width=300 #fig-citas}
